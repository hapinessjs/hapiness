...
import { WebSocketServer } from '../../../../src/extensions/socket-server/server';
...
class WebSocketRoomsTestSuite {
    ...
    testFunctions() {
        ...
        unit.array(connection2.sendUTF['firstCall']['args']).is([JSON.stringify({ event: 'message', data: { foo: 'bar' } })]);
        instance.join('room1', socket1);
