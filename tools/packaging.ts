...
class Packaging {
     ...
     */
    process() {
        Observable.forkJoin(this._files.map((fileObject: fileObject) => 
            this._copy(fileObject.name).flatMap(_ => 
                this._remove(fileObject.name, fileObject.remove)))).subscribe(null, error => 
                    console.error(error));
    }
}
